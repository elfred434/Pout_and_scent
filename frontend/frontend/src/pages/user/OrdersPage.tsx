...
const statutConfig: Record<string, { label: string; color: string; icon: typeof Clock }> = {
  ...
  EXPIREE: { label: 'Expirée', color: 'bg-neutral-100 text-neutral-700', icon: XCircle },
};
...
export function OrdersPage() {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      <h2 className="text-2xl font-bold text-neutral-900 mb-6">Mes commandes</h2>
      ...
      {orders.length > 0 ? (
                ...
                {order.lignes && order.lignes.length > 0 && (
                  <div className="border-t pt-3 space-y-2">
                    ...
                    {order.lignes.length > 3 && (
                      <p className="text-xs text-neutral-500">+ {order.lignes.length - 3} autre(s) article(s)</p>
                    )}
                  </div>
                )}
        ...
        <div className="text-center py-12">
          ...
          <p className="text-neutral-600 mb-4">Vous n'avez pas encore de commandes</p>
          <Link to="/catalogue">
