    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.variante_id} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
              <div>
                <p className="font-medium text-gray-900">{item.produit_nom}</p>
                <p className="text-sm text-gray-600">{item.contenance_ml} ml × {item.quantite}</p>
              </div>
        ...
        <div className="border-t mt-4 pt-4 flex justify-between items-center">
          ...
          <p className="text-2xl font-bold text-purple-600">{total.toLocaleString('fr-FR')} FCFA</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/profil/adresses" className="text-sm text-purple-600 hover:text-purple-700 font-medium">
            Gérer mes adresses
          </Link>
        </div>
        ...
        {addresses.length > 0 ? (
            ...
            {addresses.map((address: any) => (
              ...
              >
                ...
                <div className="ml-3 flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1">{address.quartier}, {address.ville}</p>
                  {address.indications && (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mb-4">Vous n'avez pas encore d'adresse enregistrée</p>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-gray-700">
            ...
            <Link to="/cgv" target="_blank" className="text-purple-600 hover:underline font-medium">
              Conditions Générales de Vente
            </Link>{' '}
            et la{' '}
            <Link to="/politique-confidentialite" target="_blank" className="text-purple-600 hover:underline font-medium">
      ...
      >
        {checkoutMutation.isPending ? (
          ...
          <>
            Confirmer la commande (Paiement à la livraison) — <strong>{total.toLocaleString('fr-FR')} FCFA</strong>
          </>
        )}
      </Button>
